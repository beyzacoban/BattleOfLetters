...
playFirst letters board forbiddenIndexes moveCount maxMove
  ...
  | abcWins (letters !! 3) board forbiddenIndexes = putStrLn "A & B & C win!"
  | otherwise = do
...
abcWins :: Int -> [[String]] -> [Int] -> Bool
abcWins zPos board forbidden =
  null
    [ pos
      | pos <- adjacent zPos,
        pos `notElem` forbidden,
        get pos == "-",
        isValidMove "Z" zPos pos
    ]
  where
    get i = board !! (i `div` 5) !! (i `mod` 5)

adjacent :: Int -> [Int]
adjacent i =
  [ j | j <- [i - 6, i - 5, i - 4, i - 1, i + 1, i + 4, i + 5, i + 6], j >= 0, j < 15, abs ((i `div` 5) - (j `div` 5)) <= 1, abs ((i `mod` 5) - (j `mod` 5)) <= 1
  ]
